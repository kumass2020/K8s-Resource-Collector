...
for i, element in enumerate(output_list):
    element = element.replace('namespace/', '')
...
quarter1 = int(len(resource_type_list) / 4)
quarter3 = int(len(resource_type_list) / 4 * 3)
namespace_list = namespace_list[quarter1:quarter3]
        ...
        for i, element in enumerate(output_list):
            ...
            for resource in resource_list:
                ...
                while True:
                    try:
                        try:
                            if os.stat(current_path + '/' + resource + '.yaml').st_size != 0:
                                print('already exist: ' + resource)
                                break
